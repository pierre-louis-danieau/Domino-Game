...
class Jeu(Joueurs):
    ...
    def affichage_liste_domino_tuple(self):
        """affiche la liste en ligne des dominos sur la table en format tuple"""
        L = []
        for i in range(len(self.liste_domino)):
            L.append(Domino.tuple_domino(self.liste_domino[i]))
        return(L)
    ...
    def premier_joueur(self,n_joueurs):
        ...
        for i in range(n_joueurs):
            for j in range(Joueurs.taille_pioche_joueur(self,i)):
                if Domino.double(Joueurs.pioche_joueurs(self,i)[j]):
                    double[i].append(Joueurs.pioche_joueurs(self,i)[j])
        ...
        if double[0]!=[] and double[1]!=[]:
            ...
            return(0,Joueurs.pioche_joueurs(self,0)[0]) # si personne n'a de double c'est le premier joueur qui pose son premier domino
              
    
    def permutation1(self,domino):
        """permute un domino"""
        a,b=Domino.tuple_domino(domino)
        dominopermute=Domino(self,a)
    ...
    def suivant(self,bool,num_joueur,num_domino,emplacement):  
        ...
        if bool== True:   #si le joueur peut jouer
            domino=self.pioche_joueur[num_joueur][num_domino]
            if emplacement==0:  #si c'est a gauche
                ...
                if domino.b==domino_a_cote.a:
                    ...
                    self.liste_domino.insert(0,self.permutation1(domino))      #permutation
                    
                else:   
                    print("")                                            #le joueur s'est trompé
                    print("vous vous êtes trompé")
                    print("")
                    num_domino,emplacement=self.interaction(num_joueur,bool)
                    self.suivant(bool,num_joueur,num_domino,emplacement)
                ...
                if domino.a==domino_a_cote.b:
                    ...
                    self.liste_domino.append(self.permutation1(domino))        #permutation
                    ...
                    print("")
                    num_domino,emplacement=self.interaction(num_joueur,bool)
                    self.suivant(bool,num_joueur,num_domino,emplacement)       #récursivité
                        
            
            self.pioche_joueur[num_joueur].remove(domino)  #suppression du domino dans la liste de domino du joueur
            
        else:                                           #si le joueur ne peut pas jouer
            if len(self.pioche)>1:
                domino_pioche=self.pioche[randint(0,len(self.pioche)-1)]       #il pioche un domino et le met dans sa liste
                self.pioche.remove(domino_pioche)
                self.pioche_joueur[num_joueur].append(domino_pioche)
            elif len(self.pioche)==1:                                                   #si il ne reste plus qu'un element dans la pioche
                domino_pioche=self.pioche[0]
                self.pioche.remove(domino_pioche)
                self.pioche_joueur[num_joueur].append(domino_pioche)
            else:
                return(True)
    ...
    def possible_jouer(self, num_joueur):
        ...
        print(self.liste_domino)
        if self.fini(num_joueur)==True:
            return(False)
        ...
        for i in range(self.taille_pioche_joueur(num_joueur)):
            domino=self.pioche_joueur[num_joueur][i]
            A,B=Domino.tuple_domino(domino)
    ...
    def interaction(self,num_joueur,bool):
        ...
        self.affichage_liste_domino()
        print("")
        print("la pioche du joueur {} est :".format(self.joueurs[num_joueur]))
        print("")
        self.pioche_joueurs2(num_joueur)
    ...
    def ordinateur(self,bool):
        ...
        self.affichage_liste_domino()
        print("")
        print("la pioche du joueur {} est :".format(self.joueurs[1]))
        print("")
        self.pioche_joueurs2(1)
        if bool==True:
            liste_domino_possible=[]
            
            domino_gauche,domino_droite=self.liste_domino[0], self.liste_domino[-1]
            chiffre_gauche,chiffre_droite=Domino.tuple_domino(domino_gauche)[0],Domino.tuple_domino(domino_droite)[1]
            for i in range(self.taille_pioche_joueur(1)): #on regarde dans la pioche de l'ordinateur (joueur 1)
                domino=self.pioche_joueur[1][i]
                A,B=Domino.tuple_domino(domino)
            ...
            max_somme=0
            for i in range(len(liste_domino_possible)):         # On récupère le plus grand domoino de la pioche du joueur
            ...
            num_domino=self.pioche_joueurs(1).index(domino)
            print("")
    ...
    def fini(self,num_joueur):
        ...
        if self.taille_pioche==0 and self.possible_jouer(num_joueura)==False:
            return(True)
        
        elif self.taille_pioche_joueur(0)==0:
            return(True)
            
        elif self.taille_pioche_joueur(1)==0:
            return(True)
        else:
            return(False)
    ...
    def points(self, num_joueur):
        ...
        x=0
        for i in range(self.taille_pioche_joueur(num_joueur)):
            A,B=self.pioche_joueurs_tuple(num_joueur)[i]
            x=x+A+B
        return(x)
    ...
    def vainqueur(self):
        ...
        if self.taille_pioche==0:
            points0,points1=self.points(0),self.points(1)  #nombre de points des deux joueurs
            if points0>points1:
                print("")
                print ("\033[36mLE VAINQUEUR EST {}  \033[0m".format(self.joueurs[1]))
            elif points1>points0:
                print("")
                print ("\033[36mLE VAINQUEUR EST {}  \033[0m".format(self.joueurs[0]))
            else:
                print("")
                print("MATCH NUL, AUCUN VAINQUEUR")       #match nul si ils ont le même nombre de points
        
        if self.taille_pioche_joueur(0)==0:
            print("")
            print("LE VAINQUEUR EST {}  ".format(self.joueurs[0]))
            
        if self.taille_pioche_joueur(1)==0:
            print("")
            print("LE VAINQUEUR EST {}  ".format(self.joueurs[1]))
        


    def jeu_final(self,n_joueurs,computer):
        num_joueur,domino=self.premier_joueur(n_joueurs)
        ...
        if computer==True and num_joueur==1:
            ...
            print("")
            print("Le joueur qui commence la partie est : {}  et tu es obligé de poser ton plus grand double".format(self.joueurs[num_joueur]))
            print("")
            
        self.liste_domino.append(domino)
        self.pioche_joueur[num_joueur].remove(domino)
        self.affichage_liste_domino()
        ...
        k=1
        while self.fini(num_joueur)==False:
            ...
            bool=self.possible_jouer(num_joueur) 
            ...
            if bool==True and num_joueur==1 and computer==True:
                ...
                print("\033[31m------------------------------------\033[0m")
                print("\033[31mTOUR SUIVANT {}: Le joueur {} peut jouer\033[0m".format(k,self.joueurs[num_joueur]))
                print("\033[31m------------------------------------\033[0m")
                
            else:
                print("\033[31m------------------------------------\033[0m")
                print("\033[31mTOUR SUIVANT {}: Le joueur {} ne peut pas jouer, il doit piocher\033[0m".format(k,self.joueurs[num_joueur]))
                print("\033[31m------------------------------------\033[0m")
            ...
            if computer==False or (computer==True and num_joueur==0):                                                        
                ...
                num_domino,emplacement=self.ordinateur(bool)
...
if __name__ == "__main__":
    ...
    Jeu(points_max, n_joueurs, computer).suivant(True,1,4,1)
    #Jeu(points_max,n_joueurs,computer).jeu_final(n_joueurs,computer)
